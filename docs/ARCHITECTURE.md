...
`VocalTargetEngine` is the separate musical-intent stage below correction. It evaluates fixed-size tuning spaces with arbitrary cents or frequency-ratio degrees, arbitrary repeating periods (including non-octave systems), root offsets, enable flags, degree gravity, and ascending/descending attraction. Built-in scales are immutable data over the same representation; custom spaces use the same fixed capacity and serialize with presets. It generates a causal continuous target trajectory rather than a per-frame nearest-note answer. Its continuous articulation input changes previous-target hysteresis, onset re-acquisition, and glide history: connected/legato behavior can preserve a transition through tuning space while detached/staccato behavior can reset target history at a new onset. Center correction, drift retention, and vibrato retention remain independent controls. Target search is bounded to 48 degrees across five neighboring periods with no allocation or lookahead.
