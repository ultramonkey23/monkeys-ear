...
const ModuleIdentity VocalModule::kIdentity{
    ...
    "Monkey's Ear Vocal",
    "Fx|Pitch Shift",
    ModuleKind::AudioEffect,
    1u
};

VocalModule::VocalModule() noexcept {
    // Safe and immediately audible defaults. Correction is intentionally mild;
    // listening proof, not code, decides the eventual factory sweet spot.
...
ModuleIOContract VocalModule::io_contract() const noexcept {
    ...
    io.audio_inputs = 2;
    io.audio_outputs = 2;
    io.accepts_midi = true; // reserved for explicit target control in wrapper
    io.produces_midi = false;
    io.reports_zero_latency = true;
    return io;
}
...
void VocalModule::prepare(float sample_rate, uint32_t max_block_size) noexcept {
    ...
    processor_.set_gain(0.0f);
    processor_.set_mix(1.0f);
    processor_.set_highpass_enabled(false); // standalone vocal must not silently alter dry tone
    processor_.set_vocal_controls(controls_);
    processor_.reset();
    health_ = {true, true, true};
}

void VocalModule::reset() noexcept {
    processor_.reset();
    health_.finite_output = true;
}

void VocalModule::set_controls(const VocalExpressionControls& controls) noexcept {
    controls_ = controls;
    controls_.enabled = true; // host bypass owns bypass; avoid two conflicting bypass concepts
    processor_.set_vocal_controls(controls_);
}
...
void VocalModule::process_block(const float* input_l, const float* input_r,
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        // Current vocal owner is mono analysis/resynthesis. Use mid for one
        // deterministic pitch trajectory and preserve side information dry.
        ...
        float out_l = sanitize(wet_mid + side);
        float out_r = sanitize(wet_mid - side);
        if (!std::isfinite(out_l) || !std::isfinite(out_r)) {
            out_l = l; out_r = r;
            health_.finite_output = false;
        }
        output_l[i] = out_l;
        output_r[i] = out_r;
    }
    // A host should not call above negotiated max block size. Fail safely for
    // the tail rather than reading/writing beyond our declared processing span.
