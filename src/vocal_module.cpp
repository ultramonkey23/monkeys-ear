...
void VocalModule::prepare(float sample_rate, uint32_t max_block_size) noexcept {
    ...
    processor_.set_sample_rate(sample_rate_); processor_.set_gain(0.0f); processor_.set_mix(1.0f);
...
void VocalModule::reset() noexcept { processor_.reset(); clear_ecosystem_evidence(); health_.finite_output=true; }
void VocalModule::set_controls(const VocalExpressionControls& controls) noexcept {
    controls_=controls; controls_.enabled=true; processor_.set_vocal_controls(controls_);
}
...
void VocalModule::process_block(const float* input_l,const float* input_r,float* output_l,float* output_r,uint32_t num_samples) noexcept {
    ...
    const bool use_shared=shared_pitch_valid_&&shared_pitch_confidence_>=local_pitch_confidence_;
    const float tracked_hz=use_shared?shared_pitch_hz_:local_pitch_hz_;
    const float confidence=use_shared?shared_pitch_confidence_:local_pitch_confidence_;
